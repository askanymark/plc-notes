...
; a list of types in pairs
(defvar pl-types-names
    '((OO . JAVA) (PROCEDURAL . C) (FUNCTIONAL . HASKELL) (LOGICAL . PROLOG)))
...
(defun get-pl-name ()
    ...
    (let*
        ...
        (if (pl-typep element) ;is element a valid PL name?
            ; then:
...
(let ((pl-name (get-pl-name)))
    (write-string
        (format nil "~A is of type: ~A~%" pl-name (pl-type-to-name pl-name))))
